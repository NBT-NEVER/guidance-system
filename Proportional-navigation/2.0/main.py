...
文件名: main.py
功能说明: 比例导航实验 2.0 主程序 / Main program for proportional-navigation experiment 2.0.
"""
...
from config import (
    ...
    MISSILE_POSITION_M,
    MISSILE_SPEED_MPS,
    NAVIGATION_RATIO_VALUES,
    NOISE_STD_RADPS_VALUES,
    RANDOM_SEED,
    SAVE_DIR,
    SUMMARY_CSV,
    SUMMARY_JSON,
...
def plot_trajectory(results: list[dict], target_tracks: list[dict]) -> None:
    ...
    for item, target in zip(results, target_tracks):
        ax.plot(item["missile_x"], item["missile_y"], label=f"K={item['navigation_ratio']}, σ={item['noise_std_radps']}")
...
def plot_metrics(results: list[dict]) -> None:
    ...
    for navigation_ratio in NAVIGATION_RATIO_VALUES:
        subset = [item for item in results if item["navigation_ratio"] == navigation_ratio]
        axes[0].plot(
            [item["noise_std_radps"] for item in subset],
            [item["max_required_overload_g"] for item in subset],
            marker="o",
            label=f"K={navigation_ratio}",
        )
        axes[1].plot(
            [item["noise_std_radps"] for item in subset],
            [item["miss_distance_m"] for item in subset],
            marker="s",
            label=f"K={navigation_ratio}",
        )
    ...
    axes[0].legend()
    axes[1].set_xlabel("视线角速度噪声标准差 / rad/s")
...
def main() -> None:
    ...
    for navigation_ratio in NAVIGATION_RATIO_VALUES:
        for noise_std_radps in NOISE_STD_RADPS_VALUES:
            ...
            results.append(
                {
                    ...
                    "noise_std_radps": noise_std_radps,

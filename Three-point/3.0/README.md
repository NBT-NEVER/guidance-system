# 实验三（困难）：对比目标机动条件下，三点法、前置量法与半前置量法的影响

## 1. 实验任务
当前目录对应《guidance-system.pdf》中的 **实验三（困难）：对比目标机动条件下，三点法、前置量法与半前置量法的影响**。三点法部分的教材截图在 OCR 之后有缺符号和少量错字，所以这里把核心思路保留为“地面站—目标—导弹同线约束”，再用一个写得更清楚的归一化线偏差模型落成代码。

- 实验目的：让目标做 1g 到 5g 的法向机动，比较三点法、前置量法和半前置量法在机动目标前的差别。
- 重点扫描量：目标机动过载 / g = 1，2，3，4，5
- 默认时间步长：`0.01 s`
- 当前实验的主要过载约束：`20.0 g`
...
## 8. 当前实验模式
- 模式：`law_compare_maneuver`
- 题目要求中的核心观察量：除了最大需用过载和脱靶量，这里还额外记录命中点附近的终端过载，方便看末段控制压力。

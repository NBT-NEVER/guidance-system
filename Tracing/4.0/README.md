...
1. 实验目的： 实验确定追踪法导引方法中允许攻击区范围。
2. 实验环境：自寻的制导仿真实验平台
3. 实验内容：在给定速度比和最大可用过载的情况下，遍历不同的目标相对距
离和弹目视线角， 绘出允许攻击区。
4. 实验步骤：
（1） 运行自寻的制导仿真平台，编写程序实现追踪法导引律；
（2） 实验条件设置：导弹速度V=1.2Ma,目标速度Vt=0.8Ma，速度比为1.5;
目标运动模式：水平匀速直线运动； 最大初始距离为5000m， 导弹最大可用
过载为20g；
（3）运行自寻的制导仿真实验平台， 遍历不同的初始弹目距离和初始目标
视线角， 考察追踪法导引弹道， 求出导引弹道上最大需用过载和脱靶量，根
据实验结果绘出允许攻击区。

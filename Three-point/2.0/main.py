...
文件名: main.py
功能说明: 三点法实验 2.0 主程序 / Main program for three-point experiment 2.0.
"""
...
def plot_metrics(results: list[dict]) -> None:
    ...
    for law_name in {item["law_name"] for item in results}:
        subset = sorted([item for item in results if item["law_name"] == law_name], key=lambda row: row["elevation_deg"])
        x_values = [item["elevation_deg"] for item in subset]
...
def plot_trajectory(results: list[dict], target_tracks: list[dict]) -> None:
    """
    功能：绘制代表性初始高低角下的三种方法轨迹对比图。
    参数：results 为导弹结果列表，target_tracks 为目标轨迹列表。
    返回：无。
    调用位置：main() 统计完成后。
    """

    representative = min(ELEVATION_CASES, key=lambda item: abs(item["elevation_deg"] - 45))
    selected = [item for item in results if item["elevation_deg"] == representative["elevation_deg"]]
    ...
    ax.set_xlabel("x / m")
    ax.set_ylabel("y / m")
    ax.set_title(f"{TITLE}（{representative['elevation_deg']}°工况）")
...
def main() -> None:
    ...
    for case in ELEVATION_CASES:
        for law_case in LAW_CASES:
            x_m, y_m = case["missile_position_m"]
            x_t, y_t = case["target_position_m"]
            missile_speed_mps = math.hypot(*case["missile_velocity_mps"])
            target_speed_mps = math.hypot(*case["target_velocity_mps"])
            gamma = math.atan2(case["missile_velocity_mps"][1], case["missile_velocity_mps"][0])
            target_heading = math.atan2(case["target_velocity_mps"][1], case["target_velocity_mps"][0])
            prev_line_error = 0.0

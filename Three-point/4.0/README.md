...
## 1. 实验任务
当前目录对应《guidance-system.pdf》中的 **实验四（困难）：三点法攻击禁区验证**。三点法部分的教材截图在 OCR 之后有缺符号和少量错字，所以这里把核心思路保留为“地面站—目标—导弹同线约束”，再用一个写得更清楚的归一化线偏差模型落成代码。

- 实验目的：在固定目标高度 7000m 的截面上，改变导弹初始高低角和可用过载，统计三点法的攻击禁区。
- 重点扫描量：高低角与过载约束 = e=10:2:90，nmax=1,3,5,10
- 默认时间步长：`0.01 s`
- 当前实验的主要过载约束：`10.0 g`
...
## 8. 当前实验模式
- 模式：`forbidden_zone`
- 题目要求中的核心观察量：因为题目给定了目标高度，所以代码把禁区画成“高度 7000m 截面上的可达/不可达点”。

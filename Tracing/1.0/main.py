...
文件名: main.py
功能说明: 追踪法实验一主程序 / Main program for tracing experiment 1.0.
"""
...
def plot_trajectory(results: list[dict], target_x: list[float], target_y: list[float]) -> None:
    ...
    for item in results:
        ax.plot(item["missile_x"], item["missile_y"], label=f"P={item['speed_ratio']}")
        ...
        summary_rows.append(
            {
                ...
                "missile_speed_mps": item["missile_speed_mps"],
                "target_speed_mps": item["target_speed_mps"],

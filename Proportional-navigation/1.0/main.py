...
def plot_trajectory(results: list[dict], target_x: list[float], target_y: list[float]) -> None:
    """
    功能：绘制不同导航比下的弹道对比图。
    参数：results 为实验结果列表，target_x 与 target_y 为目标轨迹坐标。
    返回：无。
    调用位置：main() 计算完成后。
    """
    ...
    for item in results:
        ax.plot(item["missile_x"], item["missile_y"], label=f"K={item['navigation_ratio']}")
    ax.plot(target_x, target_y, linestyle="--", color="black", label="目标")
    ax.set_xlabel("x / m")
    ax.set_ylabel("y / m")
    ax.set_title(TITLE)
...
def plot_metrics(results: list[dict]) -> None:
    ...
    x_values = [item["navigation_ratio"] for item in results]
...
def main() -> None:
    ...
    for navigation_ratio in NAVIGATION_RATIO_VALUES:
        ...
        if not target_x_reference:
            target_x_reference = target_x
            target_y_reference = target_y
    ...
    for item in results:
        summary_rows.append(
            {
                "navigation_ratio": item["navigation_ratio"],
    ...
    # 绘制不同导航比条件下的弹道图。
    plot_trajectory(results, target_x_reference, target_y_reference)
    # 绘制当前实验的指标统计图。

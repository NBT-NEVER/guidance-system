...
文件名: main.py
功能说明: 追踪法实验 2.0 主程序 / Main program for tracing experiment 2.0.
"""
...
def plot_trajectory(results: list[dict], target_tracks: list[dict]) -> None:
    ...
    for item, target in zip(results, target_tracks):
        ax.plot(item["missile_x"], item["missile_y"], label=f"q0={item['initial_los_deg']}°")
...
def plot_metrics(results: list[dict]) -> None:
    ...
    x_values = [item["initial_los_deg"] for item in results]
...
def main() -> None:
    ...
    for initial_los_deg in INITIAL_LOS_VALUES_DEG:
        x_m = 0.0
        y_m = 0.0
    ...
    for item in results:
        summary_rows.append(
            {
                "initial_los_deg": item["initial_los_deg"],

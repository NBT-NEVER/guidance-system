...
## 1. 实验任务
当前目录对应《guidance-system.pdf》中的 **实验一（中等）：三点法制导律参数对导引弹道特性的影响**。三点法部分的教材截图在 OCR 之后有缺符号和少量错字，所以这里把核心思路保留为“地面站—目标—导弹同线约束”，再用一个写得更清楚的归一化线偏差模型落成代码。

- 实验目的：在 3g 过载约束下，分别扫描线偏差增益、线偏差微分增益和动态误差补偿增益，观察弹道收敛区间。
- 重点扫描量：三类增益扫描 = k_e=0.1:0.1:3.0，k_d=0:0.1:2.0，k_c=0:0.1:2.0
- 默认时间步长：`0.01 s`
- 当前实验的主要过载约束：`3.0 g`
...
## 8. 当前实验模式
- 模式：`gain_sweep`
- 题目要求中的核心观察量：这里按单变量扫描处理：每次只改一类增益，另外两项固定在基准值，便于看清哪一项改变了弹道。

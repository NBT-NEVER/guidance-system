# 实验五（困难）：考虑姿态控制回路的三点法仿真

## 1. 实验任务
当前目录对应《guidance-system.pdf》中的 **实验五（困难）：考虑姿态控制回路的三点法仿真**。三点法部分的教材截图在 OCR 之后有缺符号和少量错字，所以这里把核心思路保留为“地面站—目标—导弹同线约束”，再用一个写得更清楚的归一化线偏差模型落成代码。

- 实验目的：选取一个 45° 基准阵位，对比三点法在无姿态回路和有二阶姿态回路条件下的过载与弹道差异。
- 重点扫描量：姿态回路对比 = 无姿态回路，二阶姿态回路
- 默认时间步长：`0.01 s`
- 当前实验的主要过载约束：`10.0 g`
...
## 8. 当前实验模式
- 模式：`attitude_compare`
- 题目要求中的核心观察量：题目要求“从其他任务中选取一种工况”，这里取高度 7000m、初始视线角约 45° 的基准情形做对比。
